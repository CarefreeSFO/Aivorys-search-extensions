  ...
  async function doSearch(query) {
    ...
    resultsEl.innerHTML = `
      <div class="loading">
        ...
        <div>${AivorysI18n.getMessage("searching") || "Searching..."}</div>
      </div>
    `;

    // If user prefers to open in a new tab, redirect to the SearXNG instance
    ...
    try {
      ...
      if (results.length === 0) {
        resultsEl.innerHTML = `<div class="no-results">${AivorysI18n.getMessage("noResults") || "No results found."}</div>`;
        return;
      }
  ...
  function showError(message) {
    resultsEl.innerHTML = `
      <div class="error-message">
        ${AivorysI18n.getMessage("searchError") || "Search error. Check your backend connection."}<br>
